...
from config import CVS_PATH, JDS_PATH, FEEDBACKS_PATH, validate_data_files, ensure_directories
...
def load_agent_and_feedback():
    ...
    try:
        ...
        st.info("🔧 Run 'python setup_data.py' to auto-generate sample data files")
        st.stop()
        return None, None
    except Exception as e:
        st.error(f"❌ Failed to load agent or feedback: {e}")
        st.info("🔧 Check your data files and try restarting the dashboard")
        st.stop()
        return None, None
